	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 localPositions[1023];


				for (int k = 0; k < _BloodSplatterCount; k++)
				{
					localPositions[k] = UnityObjectToClipPos(_BloodSplatterOrigins[k]);

					if (distance(localPositions[0].xy, i.uv) < gnoise(normalize(localPositions[0].xy - i.uv) * 6) / 8.0 + 0.1)
					{
